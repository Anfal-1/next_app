...
export function SoilErosionTable() {
  ...
  const filteredData = erosionData.filter(
    (item) =>
      item.location.includes(searchTerm) || item.soilType.includes(searchTerm) || item.riskLevel.includes(searchTerm),
  )
        ...
        <Table>
          <TableHeader className="bg-green-50">
            <TableRow>
              ...
              <TableHead className="text-green-800 hidden md:table-cell">نوع التربة</TableHead>
              <TableHead className="text-green-800 hidden md:table-cell">الانحدار</TableHead>
              <TableHead className="text-green-800 hidden lg:table-cell">الغطاء النباتي</TableHead>
              <TableHead className="text-green-800 hidden lg:table-cell">آخر تحديث</TableHead>
            </TableRow>
          </TableHeader>
          <TableBody>
            {filteredData.map((item) => (
              <TableRow key={item.id} className="hover:bg-green-50">
                ...
                <TableCell className="hidden md:table-cell">{item.soilType}</TableCell>
                <TableCell className="hidden md:table-cell">{item.slope}</TableCell>
                <TableCell className="hidden lg:table-cell">{item.vegetation}</TableCell>
