...
export function AIAnalysisSection() {
  ...
  const [selectedTask, setSelectedTask] = useState<AnalysisTask>("desertification")
  ...
  const handleStartAnalysis = async () => {
    ...
    try {
      ...
      const mockResult: AnalysisResult = generateMockResults(selectedTask)
      ...
      <CardContent className="p-0">
        <Tabs defaultValue="upload">
          <div className="p-4 border-b border-green-100 dark:border-gray-700 flex justify-between items-center">
            <div>
              <h2 className="text-lg font-semibold text-green-800 dark:text-green-400">{t("ai.title")}</h2>
              <p className="text-sm text-green-600 dark:text-green-500">{t("ai.subtitle")}</p>
            </div>
          ...
          <TabsContent value="upload" className="p-4">
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
              <ImageUploader selectedFile={selectedFile} onFileSelected={handleFileSelected} status={status} />
