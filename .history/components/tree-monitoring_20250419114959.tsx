...
export function TreeMonitoring() {
  ...
  const totalTreesPlanted = treePlantingData.reduce(
    (sum, item) => sum + item.trees,
    0
  )

  // Calculate yearly target
  const yearlyTarget = treePlantingData.reduce(
  ...
  const progressPercentage = Math.round(
    (totalTreesPlanted / yearlyTarget) * 100
  )

  // Sites with poor health for alerts
  const poorHealthSites = afforestationSites.filter(
    site => site.healthStatus === 'ضعيف'
  )
                ...
                <div className="flex items-center">
                  ...
                  <div>
                    <h3 className="text-lg font-semibold text-green-800">
                      إجمالي الأشجار المزروعة
                    </h3>
                    <p className="text-sm text-green-600">
                      مبادرة التشجير السنوية
                    </p>
                  </div>
                </div>
              </div>
                ...
                <div className="flex items-end justify-between">
                  <div>
                    <p className="text-4xl font-bold text-green-800">
                      {totalTreesPlanted.toLocaleString('ar-SA')}
                    </p>
                    <p className="text-sm text-green-600">
                      شجرة مزروعة هذا العام
                    </p>
                  </div>
          ...
          <CardContent className="p-6">
            <div className="flex items-center justify-between mb-4">
              <div>
                ...
                <p className="text-sm text-green-600">
                  تقدم الزراعة الشهري مقابل الأهداف
                </p>
              </div>
          ...
          <AlertDescription>
            {poorHealthSites.map(site => (
              <div key={site.id} className="mt-2">
                <span className="font-medium">{site.name}:</span> يتطلب اهتمامًا
                فوريًا. صحة الأشجار ضعيفة مع مخاطر محتملة على معدل البقاء.
              </div>
            ))}
          </AlertDescription>
        </Alert>
      ...
      <Tabs defaultValue="map" className="w-full">
        <div className="flex justify-between items-center mb-4">
          <div>
            ...
            <p className="text-sm text-green-600">
              التوزيع الجغرافي لمبادرات زراعة الأشجار
            </p>
          </div>
                  ...
                  <div className="flex justify-between items-start">
                    <div>
                      ...
                      <div className="flex items-center text-green-600 text-sm mt-1">
                        ...
                        <span>
                          الإحداثيات: {site.location[0].toFixed(2)},{' '}
                          {site.location[1].toFixed(2)}
                        </span>
                      </div>
                    </div>
                    <Badge
